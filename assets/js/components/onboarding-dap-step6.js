/**
 * myCFiA Design System - Digital Adoption Platform (DAP) Onboarding - Step 6 Upload
 *
 * Système d'onboarding guidé pour la page d'upload de contacts (Step 6)
 * - Welcome modal (première visite)
 * - Tour guidé avec spotlight et tooltips
 * - Help button pour relancer le tour
 * - Persistence via LocalStorage
 *
 * @module OnboardingDAPStep6
 * @requires Bootstrap 5 (pour modal et tooltips)
 */

/**
 * Configuration des étapes du tour guidé Step 6
 * @const {Array<Object>}
 */
const ONBOARDING_STEPS_STEP6 = [
    {
        id: "welcome",
        type: "modal",
        target: null,
    },
    {
        ...
        title: "Étape 6 : Upload de Contacts",
        content: "Vous êtes à l'étape 6 sur 8. Cette étape optionnelle vous permet d'importer vos contacts pour personnaliser vos campagnes SMS/Email/Courrier.",
    ...
    },
    {
        ...
        title: "Zone d'upload de fichier",
        content: "Glissez-déposez votre fichier CSV/Excel ici, ou cliquez sur 'Parcourir' pour sélectionner un fichier depuis votre ordinateur (max 10 MB).",
        icon: "bi-cloud-upload",
    },
    {
        id: "info-enrichment",
        type: "hotspot",
        target: ".row.g-3 .col-12:nth-child(1) .upload-info-card",
        title: "Enrichissement IA des personas",
        content: "L'IA analysera vos contacts pour générer des personas plus précis basés sur vos données clients réelles.",
        icon: "bi-magic",
    },
    {
        id: "info-security",
        type: "hotspot",
        target: ".row.g-3 .col-12:nth-child(2) .upload-info-card",
        title: "Sécurité et conformité RGPD",
        content: "Vos données sont chiffrées et automatiquement supprimées après génération de la campagne. Conformité RGPD garantie.",
        icon: "bi-shield-check",
    },
    {
        id: "info-mapping",
        type: "hotspot",
        target: ".row.g-3 .col-12:nth-child(3) .upload-info-card",
        title: "Mapping intelligent automatique",
        content: "L'IA détectera automatiquement les colonnes de votre fichier (nom, prénom, email, téléphone) pour un mapping rapide et précis.",
    ...
    },
    {
        ...
        content: "Vous pouvez passer cette étape si vous n'avez pas de contacts à importer, ou annuler pour revenir au début du workflow.",
 ...
 * @const {Object}
 */
const STORAGE_KEYS_STEP6 = {
    COMPLETED: "mycfia_onboarding_step6_completed",
    STEP: "mycfia_onboarding_step6_step",
    SKIPPED_AT: "mycfia_onboarding_step6_skipped_at",
    TOOLTIP_DISMISSED: "mycfia_onboarding_step6_tooltip_dismissed",
};

/**
 * Classe principale gérant l'onboarding DAP Step 6
 * @class OnboardingDAPStep6
 */
class OnboardingDAPStep6 {
    /**
     * Initialise l'instance OnboardingDAPStep6
     * @constructor
     */
    constructor() {
        this.steps = ONBOARDING_STEPS_STEP6;
        this.currentStep = 0;
        this.isActive = false;
        this.overlay = null;
        this.spotlight = null;
     ...
     */
    init() {
        console.log("[OnboardingDAPStep6] Initialisation...");

        // Vérifier si déjà complété
        const isCompleted = this.getFromStorage(STORAGE_KEYS_STEP6.COMPLETED);
        ...
        console.log("[OnboardingDAPStep6] Initialisé avec succès");
    }
     ...
     */
    showWelcomeModal() {
        const modalEl = document.getElementById("onboardingWelcomeModalStep6");
        if (!modalEl) {
            console.warn(
                "[OnboardingDAPStep6] Modal welcome introuvable"
            );
            return;
        }
        ...
        const startBtn = document.getElementById("startOnboardingTourStep6");
        if (startBtn) {
     ...
     */
    startTour() {
        console.log("[OnboardingDAPStep6] Démarrage du tour guidé");
    ...
    showStep(stepIndex) {
        ...
        if (!targetEl) {
            console.warn(
                `[OnboardingDAPStep6] Élément cible "${step.target}" introuvable, passage à l'étape suivante`
            );
            this.showStep(stepIndex + 1);
            return;
        }
        ...
        console.log(`[DAP Step6] Étape "${step.id}":`, {
            elementTop,
            elementBottom,
            viewportTop,
            viewportBottom,
        ...
        if (!isVisible) {
            console.log(`[DAP Step6] Scroll nécessaire pour "${step.id}" (${visibilityRatio.toFixed(0)}% visible)`);

            // Débloquer temporairement le scroll
            const wasBlocked = document.body.classList.contains('onboarding-active');
            if (wasBlocked) {
                console.log('[DAP Step6] Body bloqué ? true');
                document.body.classList.remove('onboarding-active');
                document.body.style.top = '';
                if (this._scrollPosition !== undefined) {
                    window.scrollTo(0, this._scrollPosition);
                    console.log(`[DAP Step6] Scroll débloqué, position restaurée à ${this._scrollPosition}`);
                }
            }

            // Petit délai pour que le déblocage prenne effet
            setTimeout(() => {
                console.log(`[DAP Step6] Lancement scrollIntoView pour "${step.id}"`);
                ...
                setTimeout(() => {
                    console.log('[DAP Step6] Scroll terminé, reblocage du body');
                    // Rebloquer le scroll
                    if (wasBlocked) {
                        this._scrollPosition = window.scrollY;
                        document.body.classList.add('onboarding-active');
                        document.body.style.top = `-${this._scrollPosition}px`;
                        console.log(`[DAP Step6] Body rebloqué à position ${this._scrollPosition}`);
                    }
        ...
        console.log(`[DAP Step6] Élément "${step.id}" déjà visible (${visibilityRatio.toFixed(0)}%)`);
        ...
        this.saveToStorage(STORAGE_KEYS_STEP6.STEP, stepIndex);
    }
     ...
     */
    skipTour(source) {
        console.log(`[OnboardingDAPStep6] Tour sauté depuis: ${source}`);

        // Marquer comme sauté
        this.saveToStorage(STORAGE_KEYS_STEP6.SKIPPED_AT, Date.now());
        this.saveToStorage(STORAGE_KEYS_STEP6.COMPLETED, true);
     ...
     */
    completeTour() {
        console.log("[OnboardingDAPStep6] Tour terminé avec succès");
    ...
    showCompletionMessage() {
        ...
        toast.innerHTML = `
            ...
            <div>
                ...
                <p class="mb-0 small">Vous pouvez maintenant uploader vos contacts ou passer cette étape pour continuer.</p>
            </div>
        `;
     ...
     */
    openSpeedDial() {
        console.log("[OnboardingDAPStep6] Ouverture du Speed Dial FAB");

        const speedDialContainer = document.querySelector('.speed-dial-container');
        if (!speedDialContainer) {
            console.warn("[OnboardingDAPStep6] Speed Dial container introuvable");
            return;
        }
    ...
    closeSpeedDial() {
        ...
        console.log("[OnboardingDAPStep6] Fermeture du Speed Dial FAB");
     ...
     */
    initHelpButton() {
        const helpBtn = document.getElementById("restartOnboardingTourStep6");
        if (helpBtn) {
            helpBtn.addEventListener("click", (e) => {
                e.preventDefault();
                console.log(
                    "[OnboardingDAPStep6] Relancement du tour via bouton d'aide"
                );
    ...
    saveToStorage(key, value) {
        try {
            ...
            console.error("[OnboardingDAPStep6] Erreur localStorage:", e);
        }
    }
 ...
 */
export function initOnboardingDAPStep6() {
    const onboarding = new OnboardingDAPStep6();
    onboarding.init();
}

// Export par défaut
export default OnboardingDAPStep6;
