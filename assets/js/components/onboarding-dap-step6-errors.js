/**
 * myCFiA Design System - Digital Adoption Platform (DAP) Onboarding - Step 6 Errors
 *
 * Système d'onboarding guidé pour la page d'erreurs de validation (Step 6 Errors)
 * - Welcome modal (première visite)
 * - Tour guidé avec spotlight et tooltips
 * - Help button pour relancer le tour
 * - Persistence via LocalStorage
 *
 * @module OnboardingDAPStep6Errors
 * @requires Bootstrap 5 (pour modal et tooltips)
 */

/**
 * Configuration des étapes du tour guidé Step 6 Errors
 * @const {Array<Object>}
 */
const ONBOARDING_STEPS_STEP6_ERRORS = [
    {
        id: "welcome",
        type: "modal",
        target: null,
    },
    {
        ...
        title: "Étape 6 : Validation Bloquée",
        content: "La validation de votre fichier a échoué. Ce DAP va vous guider pour comprendre et corriger les erreurs détectées.",
        icon: "bi-exclamation-triangle",
    },
    {
        id: "alert-critique",
        type: "hotspot",
        target: ".alert.alert-warning.border-warning[role='alert']",
        title: "15 erreurs critiques détectées",
        content: "Cette alerte indique le nombre total d'erreurs bloquant la poursuite du workflow. Vous devez toutes les corriger avant de continuer.",
    ...
    },
    {
        ...
        type: "hotspot",
        target: ".row.g-3.mb-4",
        title: "Résumé des erreurs par type",
        content: "Visualisez rapidement les 3 types d'erreurs détectés : 8 emails invalides, 5 champs requis manquants, et 2 erreurs de format.",
        icon: "bi-pie-chart",
    },
    {
        id: "detailed-errors",
        ...
        title: "Liste détaillée des erreurs",
        content: "Consultez chaque erreur avec le numéro de ligne, la valeur incorrecte, le problème identifié et la solution suggérée par l'IA.",
        icon: "bi-list-ul",
    },
    {
        id: "recommended-actions",
        type: "hotspot",
        target: ".card.border-primary",
        title: "Actions recommandées",
        content: "Suivez ces 3 étapes : téléchargez le rapport d'erreurs CSV, corrigez votre fichier source, puis réuploadez-le.",
    ...
    },
    {
        ...
        content: "Téléchargez le rapport détaillé ou retournez à l'upload pour réessayer avec un fichier corrigé.",
 ...
 * @const {Object}
 */
const STORAGE_KEYS_STEP6_ERRORS = {
    COMPLETED: "mycfia_onboarding_step6_errors_completed",
    STEP: "mycfia_onboarding_step6_errors_step",
    SKIPPED_AT: "mycfia_onboarding_step6_errors_skipped_at",
    TOOLTIP_DISMISSED: "mycfia_onboarding_step6_errors_tooltip_dismissed",
};

/**
 * Classe principale gérant l'onboarding DAP Step 6 Errors
 * @class OnboardingDAPStep6Errors
 */
class OnboardingDAPStep6Errors {
    /**
     * Initialise l'instance OnboardingDAPStep6Errors
     * @constructor
     */
    constructor() {
        this.steps = ONBOARDING_STEPS_STEP6_ERRORS;
        this.currentStep = 0;
        this.isActive = false;
        this.overlay = null;
        this.spotlight = null;
     ...
     */
    init() {
        console.log("[OnboardingDAPStep6Errors] Initialisation...");

        // Vérifier si déjà complété
        const isCompleted = this.getFromStorage(STORAGE_KEYS_STEP6_ERRORS.COMPLETED);
        ...
        console.log("[OnboardingDAPStep6Errors] Initialisé avec succès");
    }
     ...
     */
    showWelcomeModal() {
        const modalEl = document.getElementById("onboardingWelcomeModalStep6Errors");
        if (!modalEl) {
            console.warn(
                "[OnboardingDAPStep6Errors] Modal welcome introuvable"
            );
            return;
        }
        ...
        const startBtn = document.getElementById("startOnboardingTourStep6Errors");
        if (startBtn) {
     ...
     */
    startTour() {
        console.log("[OnboardingDAPStep6Errors] Démarrage du tour guidé");
    ...
    showStep(stepIndex) {
        ...
        if (!targetEl) {
            console.warn(
                `[OnboardingDAPStep6Errors] Élément cible "${step.target}" introuvable, passage à l'étape suivante`
            );
            this.showStep(stepIndex + 1);
            return;
        }
        ...
        console.log(`[DAP Step6Errors] Étape "${step.id}":`, {
            elementTop,
            elementBottom,
            viewportTop,
            viewportBottom,
        ...
        if (!isVisible) {
            console.log(`[DAP Step6Errors] Scroll nécessaire pour "${step.id}" (${visibilityRatio.toFixed(0)}% visible)`);

            // Débloquer temporairement le scroll
            const wasBlocked = document.body.classList.contains('onboarding-active');
            if (wasBlocked) {
                console.log('[DAP Step6Errors] Body bloqué ? true');
                document.body.classList.remove('onboarding-active');
                document.body.style.top = '';
                if (this._scrollPosition !== undefined) {
                    window.scrollTo(0, this._scrollPosition);
                    console.log(`[DAP Step6Errors] Scroll débloqué, position restaurée à ${this._scrollPosition}`);
                }
            }

            // Petit délai pour que le déblocage prenne effet
            setTimeout(() => {
                console.log(`[DAP Step6Errors] Lancement scrollIntoView pour "${step.id}"`);
                ...
                setTimeout(() => {
                    console.log('[DAP Step6Errors] Scroll terminé, reblocage du body');
                    // Rebloquer le scroll
                    if (wasBlocked) {
                        this._scrollPosition = window.scrollY;
                        document.body.classList.add('onboarding-active');
                        document.body.style.top = `-${this._scrollPosition}px`;
                        console.log(`[DAP Step6Errors] Body rebloqué à position ${this._scrollPosition}`);
                    }
        ...
        console.log(`[DAP Step6Errors] Élément "${step.id}" déjà visible (${visibilityRatio.toFixed(0)}%)`);
        ...
        this.saveToStorage(STORAGE_KEYS_STEP6_ERRORS.STEP, stepIndex);
    }
     ...
     */
    skipTour(source) {
        console.log(`[OnboardingDAPStep6Errors] Tour sauté depuis: ${source}`);

        // Marquer comme sauté
        this.saveToStorage(STORAGE_KEYS_STEP6_ERRORS.SKIPPED_AT, Date.now());
        this.saveToStorage(STORAGE_KEYS_STEP6_ERRORS.COMPLETED, true);
     ...
     */
    completeTour() {
        console.log("[OnboardingDAPStep6Errors] Tour terminé avec succès");
    ...
    showCompletionMessage() {
        ...
        toast.innerHTML = `
            ...
            <div>
                ...
                <p class="mb-0 small">Téléchargez le rapport d'erreurs, corrigez votre fichier et réessayez l'upload.</p>
            </div>
        `;
     ...
     */
    openSpeedDial() {
        console.log("[OnboardingDAPStep6Errors] Ouverture du Speed Dial FAB");

        const speedDialContainer = document.querySelector('.speed-dial-container');
        if (!speedDialContainer) {
            console.warn("[OnboardingDAPStep6Errors] Speed Dial container introuvable");
            return;
        }
    ...
    closeSpeedDial() {
        ...
        console.log("[OnboardingDAPStep6Errors] Fermeture du Speed Dial FAB");
     ...
     */
    initHelpButton() {
        const helpBtn = document.getElementById("restartOnboardingTourStep6Errors");
        if (helpBtn) {
            helpBtn.addEventListener("click", (e) => {
                e.preventDefault();
                console.log(
                    "[OnboardingDAPStep6Errors] Relancement du tour via bouton d'aide"
                );
    ...
    saveToStorage(key, value) {
        try {
            ...
            console.error("[OnboardingDAPStep6Errors] Erreur localStorage:", e);
        }
    }
 ...
 */
export function initOnboardingDAPStep6Errors() {
    const onboarding = new OnboardingDAPStep6Errors();
    onboarding.init();
}

// Export par défaut
export default OnboardingDAPStep6Errors;
