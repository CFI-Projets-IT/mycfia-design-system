/**
 * myCFiA Design System - Digital Adoption Platform (DAP) Onboarding - Step 6 Mapping
 *
 * Système d'onboarding guidé pour la page de mapping manuel (Step 6 Mapping)
 * - Welcome modal (première visite)
 * - Tour guidé avec spotlight et tooltips
 * - Help button pour relancer le tour
 * - Persistence via LocalStorage
 *
 * @module OnboardingDAPStep6Mapping
 * @requires Bootstrap 5 (pour modal et tooltips)
 */

/**
 * Configuration des étapes du tour guidé Step 6 Mapping
 * @const {Array<Object>}
 */
const ONBOARDING_STEPS_STEP6_MAPPING = [
    {
        id: "welcome",
        type: "modal",
        target: null,
    },
    {
        ...
        title: "Étape 6/8 : Upload de contacts",
        content: "Vous êtes à l'étape 6 sur 8 du processus de création de campagne. Cette étape concerne le <strong>mapping manuel des colonnes</strong> de votre fichier vers les champs CFI.",
        icon: "bi-list-ol",
    },
    {
        id: "compteurs-mapping",
        type: "hotspot",
        target: ".d-flex.gap-3 .badge",
        title: "Statut du mapping",
        content: "Ces badges indiquent le <strong>statut global du mapping</strong> :<br>• <span class='text-success'><i class='bi bi-check-circle'></i> <strong>8 mappées</strong></span> : colonnes correctement associées<br>• <span class='text-warning'><i class='bi bi-exclamation-circle'></i> <strong>2 requises manquantes</strong></span> : champs obligatoires non mappés (bloque la validation)",
        icon: "bi-pie-chart-fill",
    },
    {
        id: "aide-legende",
        type: "hotspot",
        target: ".alert.alert-light.border[role='alert']",
        title: "Légende des statuts",
        content: "Ce guide vous aide à comprendre les <strong>4 statuts possibles</strong> :<br>• <i class='bi bi-exclamation-triangle text-danger'></i> <strong>Requis</strong> : champs obligatoires (mail, nom, prénom)<br>• <i class='bi bi-info-circle text-secondary'></i> <strong>Optionnel</strong> : champs facultatifs (téléphone, adresse, etc.)<br>• <i class='bi bi-check-circle text-success'></i> <strong>Mappé</strong> : correspondance établie<br>• <i class='bi bi-x-circle text-secondary'></i> <strong>Ignoré</strong> : colonne non utilisée",
        icon: "bi-info-square-fill",
    },
    {
        id: "ligne-erreur",
        type: "hotspot",
        target: ".mapping-row-error",
        title: "Exemple : Mapping requis manquant",
        content: "Cette ligne <strong class='text-danger'>en rouge</strong> signale un <strong>problème bloquant</strong> :<br>• La colonne \"Nom complet\" n'est pas mappée<br>• Les champs <strong>\"nom\"</strong> et <strong>\"prénom\"</strong> sont <span class='text-danger'><strong>requis</strong></span><br>• Le <strong>feedback rouge</strong> explique l'erreur<br><br>➡️ Vous devez mapper cette colonne pour continuer.",
        icon: "bi-exclamation-octagon-fill",
    },
    {
        id: "card-mapping",
        type: "hotspot",
        target: ".card.shadow-sm",
        title: "Tableau des correspondances",
        content: "Ce tableau présente <strong>toutes les colonnes</strong> de votre fichier (12 au total) :<br>• <strong>Colonne source</strong> : nom et statistiques (valeurs, vides)<br>• <strong>Dropdown CFI</strong> : sélectionnez le champ de destination<br>• <strong>Statut visuel</strong> : icône colorée (succès/erreur/ignoré)<br>• <strong>Action</strong> : boutons ignorer/mapper<br><br><i class='bi bi-grip-vertical'></i> Utilisez l'icône grip pour <strong>réordonner les lignes</strong>.",
        icon: "bi-table",
    },
    {
        id: "speed-dial-fab",
        ...
        content: "Le <strong>Speed Dial</strong> vous propose 2 actions :<br>• <i class='bi bi-arrow-counterclockwise'></i> <strong>Retour : Suggestions</strong> - revenir aux suggestions IA<br>• <i class='bi bi-check-circle'></i> <strong>Valider et continuer</strong> - passer à la preview (si mapping complet)<br><br><i class='bi bi-exclamation-triangle text-warning'></i> La validation est <strong>bloquée</strong> tant que les champs requis ne sont pas mappés.",
 ...
 * @const {Object}
 */
const STORAGE_KEYS_STEP6_MAPPING = {
    COMPLETED: "mycfia_onboarding_step6_mapping_completed",
    STEP: "mycfia_onboarding_step6_mapping_step",
    SKIPPED_AT: "mycfia_onboarding_step6_mapping_skipped_at",
    TOOLTIP_DISMISSED: "mycfia_onboarding_step6_mapping_tooltip_dismissed",
};

/**
 * Classe principale gérant l'onboarding DAP Step 6 Errors
 * @class OnboardingDAPStep6Mapping
 */
class OnboardingDAPStep6Mapping {
    /**
     * Initialise l'instance OnboardingDAPStep6Mapping
     * @constructor
     */
    constructor() {
        this.steps = ONBOARDING_STEPS_STEP6_MAPPING;
        this.currentStep = 0;
        this.isActive = false;
        this.overlay = null;
        this.spotlight = null;
     ...
     */
    init() {
        console.log("[OnboardingDAPStep6Mapping] Initialisation...");

        // Vérifier si déjà complété
        const isCompleted = this.getFromStorage(STORAGE_KEYS_STEP6_MAPPING.COMPLETED);
        ...
        console.log("[OnboardingDAPStep6Mapping] Initialisé avec succès");
    }
     ...
     */
    showWelcomeModal() {
        const modalEl = document.getElementById("onboardingWelcomeModalStep6Mapping");
        if (!modalEl) {
            console.warn(
                "[OnboardingDAPStep6Mapping] Modal welcome introuvable"
            );
            return;
        }
        ...
        const startBtn = document.getElementById("startOnboardingTourStep6Mapping");
        if (startBtn) {
     ...
     */
    startTour() {
        console.log("[OnboardingDAPStep6Mapping] Démarrage du tour guidé");
    ...
    showStep(stepIndex) {
        ...
        if (!targetEl) {
            console.warn(
                `[OnboardingDAPStep6Mapping] Élément cible "${step.target}" introuvable, passage à l'étape suivante`
            );
            this.showStep(stepIndex + 1);
            return;
        }
        ...
        console.log(`[DAP Step6Mapping] Étape "${step.id}":`, {
            elementTop,
            elementBottom,
            viewportTop,
            viewportBottom,
        ...
        if (!isVisible) {
            console.log(`[DAP Step6Mapping] Scroll nécessaire pour "${step.id}" (${visibilityRatio.toFixed(0)}% visible)`);

            // Débloquer temporairement le scroll
            const wasBlocked = document.body.classList.contains('onboarding-active');
            if (wasBlocked) {
                console.log('[DAP Step6Mapping] Body bloqué ? true');
                document.body.classList.remove('onboarding-active');
                document.body.style.top = '';
                if (this._scrollPosition !== undefined) {
                    window.scrollTo(0, this._scrollPosition);
                    console.log(`[DAP Step6Mapping] Scroll débloqué, position restaurée à ${this._scrollPosition}`);
                }
            }

            // Petit délai pour que le déblocage prenne effet
            setTimeout(() => {
                console.log(`[DAP Step6Mapping] Lancement scrollIntoView pour "${step.id}"`);
                ...
                setTimeout(() => {
                    console.log('[DAP Step6Mapping] Scroll terminé, reblocage du body');
                    // Rebloquer le scroll
                    if (wasBlocked) {
                        this._scrollPosition = window.scrollY;
                        document.body.classList.add('onboarding-active');
                        document.body.style.top = `-${this._scrollPosition}px`;
                        console.log(`[DAP Step6Mapping] Body rebloqué à position ${this._scrollPosition}`);
                    }
        ...
        console.log(`[DAP Step6Mapping] Élément "${step.id}" déjà visible (${visibilityRatio.toFixed(0)}%)`);
        ...
        this.saveToStorage(STORAGE_KEYS_STEP6_MAPPING.STEP, stepIndex);
    }
     ...
     */
    skipTour(source) {
        console.log(`[OnboardingDAPStep6Mapping] Tour sauté depuis: ${source}`);

        // Marquer comme sauté
        this.saveToStorage(STORAGE_KEYS_STEP6_MAPPING.SKIPPED_AT, Date.now());
        this.saveToStorage(STORAGE_KEYS_STEP6_MAPPING.COMPLETED, true);
     ...
     */
    completeTour() {
        console.log("[OnboardingDAPStep6Mapping] Tour terminé avec succès");
    ...
    showCompletionMessage() {
        ...
        toast.innerHTML = `
            ...
            <div>
                ...
                <p class="mb-0 small">Mappez toutes les colonnes requises, puis validez pour passer à la preview.</p>
            </div>
        `;
     ...
     */
    openSpeedDial() {
        console.log("[OnboardingDAPStep6Mapping] Ouverture du Speed Dial FAB");

        const speedDialContainer = document.querySelector('.speed-dial-container');
        if (!speedDialContainer) {
            console.warn("[OnboardingDAPStep6Mapping] Speed Dial container introuvable");
            return;
        }
    ...
    closeSpeedDial() {
        ...
        console.log("[OnboardingDAPStep6Mapping] Fermeture du Speed Dial FAB");
     ...
     */
    initHelpButton() {
        const helpBtn = document.getElementById("restartOnboardingTourStep6Mapping");
        if (helpBtn) {
            helpBtn.addEventListener("click", (e) => {
                e.preventDefault();
                console.log(
                    "[OnboardingDAPStep6Mapping] Relancement du tour via bouton d'aide"
                );
    ...
    saveToStorage(key, value) {
        try {
            ...
            console.error("[OnboardingDAPStep6Mapping] Erreur localStorage:", e);
        }
    }
 ...
 */
export function initOnboardingDAPStep6Mapping() {
    const onboarding = new OnboardingDAPStep6Mapping();
    onboarding.init();
}

// Export par défaut
export default OnboardingDAPStep6Mapping;
