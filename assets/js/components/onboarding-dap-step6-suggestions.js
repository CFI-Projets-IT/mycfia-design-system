/**
 * myCFiA Design System - Digital Adoption Platform (DAP) Onboarding - Step 6 Suggestions
 *
 * Système d'onboarding guidé pour la page de suggestions de mapping IA (Step 6 Suggestions)
 * - Welcome modal (première visite)
 * - Tour guidé avec spotlight et tooltips
 * - Help button pour relancer le tour
 * - Persistence via LocalStorage
 *
 * @module OnboardingDAPStep6Suggestions
 * @requires Bootstrap 5 (pour modal et tooltips)
 */

/**
 * Configuration des étapes du tour guidé Step 6 Suggestions
 * @const {Array<Object>}
 */
const ONBOARDING_STEPS_STEP6_SUGGESTIONS = [
    {
        id: "welcome",
        type: "modal",
        target: null,
    },
    {
        ...
        title: "Étape 6 : Suggestions de Mapping IA",
        content: "L'IA a analysé automatiquement votre fichier et détecté 9/12 colonnes. Ce DAP va vous guider pour valider et ajuster le mapping proposé.",
        icon: "bi-magic",
    },
    {
        id: "confidence-badge",
        type: "hotspot",
        target: ".badge.bg-success.fs-5",
        title: "95% de confiance IA",
        content: "L'IA a analysé vos données avec un haut niveau de confiance. Ce score indique la qualité de la détection automatique des colonnes.",
        icon: "bi-award",
    },
    {
        id: "file-info",
        type: "hotspot",
        target: ".card.border.mb-0",
        title: "Informations du fichier uploadé",
        content: "Récapitulatif de votre fichier : 1,523 lignes, 12 colonnes. L'IA a identifié automatiquement 9 correspondances sur 12 colonnes.",
        icon: "bi-file-earmark-spreadsheet",
    },
    {
        id: "alert-incomplete",
        type: "hotspot",
        target: ".alert.alert-warning[role='alert']",
        title: "Mapping incomplet - Action requise",
        content: "Certains champs requis (comme 'prénom') n'ont pas pu être mappés automatiquement. Vous devrez ajuster manuellement.",
    ...
    },
    {
        ...
        content: "Consultez chaque correspondance proposée par l'IA : 8 parfaites (vert), 2 à ajuster (jaune), 1 ignorée (gris), 1 requise manquante (rouge).",
    ...
    },
    {
        ...
        content: "Vous pouvez retourner à l'upload, ajuster manuellement le mapping, ou continuer (désactivé tant que champs requis manquants).",
 ...
 * @const {Object}
 */
const STORAGE_KEYS_STEP6_SUGGESTIONS = {
    COMPLETED: "mycfia_onboarding_step6_suggestions_completed",
    STEP: "mycfia_onboarding_step6_suggestions_step",
    SKIPPED_AT: "mycfia_onboarding_step6_suggestions_skipped_at",
    TOOLTIP_DISMISSED: "mycfia_onboarding_step6_suggestions_tooltip_dismissed",
};

/**
 * Classe principale gérant l'onboarding DAP Step 6 Suggestions
 * @class OnboardingDAPStep6Suggestions
 */
class OnboardingDAPStep6Suggestions {
    /**
     * Initialise l'instance OnboardingDAPStep6Suggestions
     * @constructor
     */
    constructor() {
        this.steps = ONBOARDING_STEPS_STEP6_SUGGESTIONS;
        this.currentStep = 0;
        this.isActive = false;
        this.overlay = null;
        this.spotlight = null;
     ...
     */
    init() {
        console.log("[OnboardingDAPStep6Suggestions] Initialisation...");

        // Vérifier si déjà complété
        const isCompleted = this.getFromStorage(STORAGE_KEYS_STEP6_SUGGESTIONS.COMPLETED);
        ...
        console.log("[OnboardingDAPStep6Suggestions] Initialisé avec succès");
    }
     ...
     */
    showWelcomeModal() {
        const modalEl = document.getElementById("onboardingWelcomeModalStep6Suggestions");
        if (!modalEl) {
            console.warn(
                "[OnboardingDAPStep6Suggestions] Modal welcome introuvable"
            );
            return;
        }
        ...
        const startBtn = document.getElementById("startOnboardingTourStep6Suggestions");
        if (startBtn) {
     ...
     */
    startTour() {
        console.log("[OnboardingDAPStep6Suggestions] Démarrage du tour guidé");
    ...
    showStep(stepIndex) {
        ...
        if (!targetEl) {
            console.warn(
                `[OnboardingDAPStep6Suggestions] Élément cible "${step.target}" introuvable, passage à l'étape suivante`
            );
            this.showStep(stepIndex + 1);
            return;
        }
        ...
        console.log(`[DAP Step6Suggestions] Étape "${step.id}":`, {
            elementTop,
            elementBottom,
            viewportTop,
            viewportBottom,
        ...
        if (!isVisible) {
            console.log(`[DAP Step6Suggestions] Scroll nécessaire pour "${step.id}" (${visibilityRatio.toFixed(0)}% visible)`);

            // Débloquer temporairement le scroll
            const wasBlocked = document.body.classList.contains('onboarding-active');
            if (wasBlocked) {
                console.log('[DAP Step6Suggestions] Body bloqué ? true');
                document.body.classList.remove('onboarding-active');
                document.body.style.top = '';
                if (this._scrollPosition !== undefined) {
                    window.scrollTo(0, this._scrollPosition);
                    console.log(`[DAP Step6Suggestions] Scroll débloqué, position restaurée à ${this._scrollPosition}`);
                }
            }

            // Petit délai pour que le déblocage prenne effet
            setTimeout(() => {
                console.log(`[DAP Step6Suggestions] Lancement scrollIntoView pour "${step.id}"`);
                ...
                setTimeout(() => {
                    console.log('[DAP Step6Suggestions] Scroll terminé, reblocage du body');
                    // Rebloquer le scroll
                    if (wasBlocked) {
                        this._scrollPosition = window.scrollY;
                        document.body.classList.add('onboarding-active');
                        document.body.style.top = `-${this._scrollPosition}px`;
                        console.log(`[DAP Step6Suggestions] Body rebloqué à position ${this._scrollPosition}`);
                    }
        ...
        console.log(`[DAP Step6Suggestions] Élément "${step.id}" déjà visible (${visibilityRatio.toFixed(0)}%)`);
        ...
        this.saveToStorage(STORAGE_KEYS_STEP6_SUGGESTIONS.STEP, stepIndex);
    }
     ...
     */
    skipTour(source) {
        console.log(`[OnboardingDAPStep6Suggestions] Tour sauté depuis: ${source}`);

        // Marquer comme sauté
        this.saveToStorage(STORAGE_KEYS_STEP6_SUGGESTIONS.SKIPPED_AT, Date.now());
        this.saveToStorage(STORAGE_KEYS_STEP6_SUGGESTIONS.COMPLETED, true);
     ...
     */
    completeTour() {
        console.log("[OnboardingDAPStep6Suggestions] Tour terminé avec succès");
    ...
    showCompletionMessage() {
        ...
        toast.innerHTML = `
            ...
            <div>
                ...
                <p class="mb-0 small">Ajustez manuellement le mapping pour mapper les champs requis manquants.</p>
            </div>
        `;
     ...
     */
    openSpeedDial() {
        console.log("[OnboardingDAPStep6Suggestions] Ouverture du Speed Dial FAB");

        const speedDialContainer = document.querySelector('.speed-dial-container');
        if (!speedDialContainer) {
            console.warn("[OnboardingDAPStep6Suggestions] Speed Dial container introuvable");
            return;
        }
    ...
    closeSpeedDial() {
        ...
        console.log("[OnboardingDAPStep6Suggestions] Fermeture du Speed Dial FAB");
     ...
     */
    initHelpButton() {
        const helpBtn = document.getElementById("restartOnboardingTourStep6Suggestions");
        if (helpBtn) {
            helpBtn.addEventListener("click", (e) => {
                e.preventDefault();
                console.log(
                    "[OnboardingDAPStep6Suggestions] Relancement du tour via bouton d'aide"
                );
    ...
    saveToStorage(key, value) {
        try {
            ...
            console.error("[OnboardingDAPStep6Suggestions] Erreur localStorage:", e);
        }
    }
 ...
 */
export function initOnboardingDAPStep6Suggestions() {
    const onboarding = new OnboardingDAPStep6Suggestions();
    onboarding.init();
}

// Export par défaut
export default OnboardingDAPStep6Suggestions;
