...
import { initCompetitorSelector } from './components/step3-competitor-selector.js';
    ...
    try {
        ...
        if (document.getElementById('channelChart')) {
            ...
            if (typeof Chart !== 'undefined') {
                ...
                console.warn('[main] Chart.js non chargé, graphiques analytics non initialisés');
            }
        }

        // 7. Initialiser le sélecteur d'assets (si présent ET pas de personas/competitors)
        if (document.querySelector('.asset-card') && !document.querySelector('.persona-card') && !document.querySelector('.competitor-card')) {
        ...
        // 9. Initialiser le sélecteur de concurrents (si présent sur step3_validate, mais pas sur personas)
        if (document.getElementById('selectedCount') && document.querySelector('.asset-card') && !document.querySelector('.persona-card')) {
        ...
        if (document.querySelector('.loader-container') && document.title.includes('Génération Assets')) {
            console.log('[main] Initialisation du loading Step 5...');
            initStep5Loading();
        }

        // 12. Initialiser la validation des assets Step 5 (si présent)
        if (document.querySelector('.asset-row[data-asset-id]')) {
            console.log('[main] Initialisation de la validation des assets Step 5...');
            initAssetValidation();
        }
